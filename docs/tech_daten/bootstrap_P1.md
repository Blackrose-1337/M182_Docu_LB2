...
- Das Skript setzt die DNS-Einstellungen auf Google DNS (8.8.8.8 und 8.8.4.4)
- Es aktualisiert auch die /etc/resolv.conf-Datei.
...
## Systemaktualisierung und -installation:
- Setzt die Umgebungsvariable DEBIAN_FRONTEND auf noninteractive, um eine nicht-interaktive Installation von Paketen zu ermöglichen.
- Konfiguriert apt-fast für eine schnellere Paketinstallation.
- Fügt zusätzliche apt-Repositorys für verschiedene Pakete hinzu.
- Installiert verschiedene Abhängigkeiten und Tools, darunter jq, whois, build-essential, git, unzip, htop, yq, mysql-server, redis-server und python-pip.
...
## Anpassung des MOTD (Message of the Day):
- Erzwingt farbigen Terminal-Output. 
- Entfernt einige Standard-MOTD-Inhalte. 
- Kopiert ein spezifisches DetectionLab-MOTD-Skript.
...
## Testen von Prerequisites:
- Überprüft, ob verschiedene Pakete erfolgreich installiert wurden.
...
## Fixierung der eth1-IP-Adresse:
- Setzt eine statische DHCP-Lease für eth1, um Probleme mit dhclient zu umgehen.
- Überprüft und korrigiert die IP-Adresse von eth1.
...
## Herunterladen der Palantir osquery-Konfiguration:
- Klont das Palantir osquery-Konfigurationsrepository, wenn es noch nicht vorhanden ist.
...
## Installation und Konfiguration von Fleet (Security Automation Platform):
- Installiert Fleet, eine Osquery-Management-Lösung.
- Konfiguriert Fleet und setzt die Datenbank auf.
...
- Installiert und konfiguriert Zeek (früher Bro), ein Netzwerküberwachungsframework.
...
## Installation von Velociraptor:
- Installiert Velociraptor, ein Endpoint-Detection-and-Response (EDR)-Tool.
...
- Installiert Suricata, ein Open-Source-Intrusion-Detection-System (IDS) und Intrusion-Prevention-System (IPS).
...
- Installiert Apache Guacamole, eine Remote-Desktop-Gateway-Lösung.
...
- Fügt Splunk und Zeek zum PATH hinzu.
